...
text( -.1 , .9*max(y) , bquote( paste(mu ," = " ,.(meanval)) )
      , adj=c(1,.5) )
text( -.1 , .8*max(y) , bquote( paste(sigma ," = " ,.(sdval)) )
      , adj=c(1,.5) )
text( .1 , .9*max(y) , bquote( paste(Delta , "x = " ,.(dx)) )
      , adj=c(0,.5) )
text( .04 , .8*max(y) ,
      bquote(
        paste( sum(,x,) , " " , Delta , "x p(x) = " , .(signif(area,3)) )
      ) , adj=c(0,.5) )
#Part B: As can be seen in the graph, roughly 70% of the data falls between one standard deviation above and below the mean.
#If we know about 2/3 of women's height falls between 147 and 177 that means we should assign a mean value of 162 cm, with a standard deviation of 15 cm. If this is done we will have replicated the data we wanted.